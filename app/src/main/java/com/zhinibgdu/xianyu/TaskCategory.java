...
public enum TaskCategory {
    ALL("自动任务"), POLISH("一键擦亮"), LOCAL("闲鱼本地任务"), VIDEO("视频任务"), GAME("小游戏任务");
    ...
    static TaskCategory classify(String title) {
        ...
        String n = title.replaceAll("\\s+", "")
                .replace("還想", "还想").replace("點點", "点点")
                .replace("領", "领").replace("獎", "奖")
                .replace("妈蚁", "蚂蚁").replace("访间", "访问")
        ...
        // Mini-games are isolated from local/video tasks.
        ...
        if (has(n, "签到", "点击指定频道好物", "指定频道好物",
                ...
                "领取闲鱼币", "闲鱼币", "搜一搜", "搜索", "搜商品",
                "逛逛商城领超值优惠券", "逛逛商城", "领超值优惠券")
                || (n.contains("逛逛") && n.contains("优惠券"))) return LOCAL;
